...
public:
    ...
    void insert(unsigned lb, unsigned ub, const Line& line)
    {
        insert(1, 0, size_, lb, ub, line);
    }
...
private:
    void insert(unsigned index, unsigned range_begin, unsigned range_end, unsigned lb, unsigned ub, const Line& line)
    {
        if (range_end <= lb || ub < range_begin)
            return;

        if (lb <= range_begin && ub + 1 >= range_end) {
            insert(index, range_begin, range_end, line);
            return;
        }

        const unsigned mid = (range_begin + range_end) / 2;
        insert(index<<1|0, range_begin, mid, lb, ub, line);
        insert(index<<1|1, mid, range_end, lb, ub, line);
    }
...
int main()
{
    ...
    for (unsigned i = 0; i < n; ++i) {
        ...
        if (t == 1) {
            ...
            lt.insert(lb, ub, Line(a, b));
        } else {
            unsigned x;
            std::cin >> x;

            const long long v = lt.query(x);
            if (v == LONG_LONG_MIN)
                std::cout << "NO" << '\n';
            else
                std::cout << v << '\n';
